...
const IAPage = () => {
    ...
    return (
        <div className="flex flex-[4_4_0] border-l border-r border-gray-700 items-center justify-center min-h-screen flex-col bg-black-900 text-white px-4">
            {!showChat ? (
                ...
                <div className="w-full max-w-lg flex flex-col bg-black-800 rounded-lg shadow-lg h-full relative">
                    ...
                    <div className="flex-1 overflow-y-auto mb-4 space-y-4 p-4 pr-2">
                        {messages.map((msg, index) => (
                            <div
                                key={index}
                                className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} items-center`} // Alinhando imagem e mensagem
                            >
                                {msg.role === "model" && (
                                    <img
                                        src="images/ia-icon.webp" // Caminho da imagem
                                        alt="IA Icon"
                                        className="w-10 h-10 mr-3 rounded-full" // Ajuste de estilo para a imagem
                                    />
                                )}
